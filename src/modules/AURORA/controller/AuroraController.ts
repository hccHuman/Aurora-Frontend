...
export function applyAuroraInstruction(
  ...
  if (!modelRef.current) {
    console.warn("⚠️ No Live2D model loaded to apply instruction.");
    return;
  }
  ...
  try {
    ...
    if (motion) {
      ...
      console.log("▶️ Executing motion (url):", motionUrl);
      try {
        ...
        console.warn(
          "⚠️ Could not play motion with motionManager, trying fallback:",
          e
        );
        try {
          ...
          console.error("❌ Error playing motion (fallback):", ee);
        }
      }
    }
      ...
      try {
        ...
        if (em && typeof em.setExpression === "function") {
          ...
          em.setExpression(expUrl).catch?.(() =>
            console.warn("⚠️ Expression not found:", expression)
          );
        } else {
          // No expressionManager available: caller should use params or motions as fallback
          console.debug(
            "ℹ️ expressionManager not available — use params or motions for expressions."
          );
        }
      } catch (err) {
        console.warn("⚠️ Error applying expression:", err);
      }
    }
  } catch (err) {
    console.error("❌ Error applying Aurora instruction:", err);
  }
}
