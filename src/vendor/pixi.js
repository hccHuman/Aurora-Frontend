...
(function() {
    try {
        if (typeof window !== 'undefined') {
            if (!window.PIXI) window.PIXI = {};
            if (!window.PIXI.settings) window.PIXI.settings = {};
            // Polyfill RETINA_PREFIX
            if (!window.PIXI.settings.RETINA_PREFIX) {
                window.PIXI.settings.RETINA_PREFIX = /@2x/;
            }
        }
    } catch (e) { console.error('PIXI Patch Error', e); }
})();
// --- PATCH END ---
